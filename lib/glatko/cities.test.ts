...
describe("toCitySlug", () => {
  ...
  /**
   * notifyProfessionalsOfNewRequest() compares a request's `municipality` (the
   * request form posts the i18n KEY) against a provider's `location_city` (the
   * SLUG). Routing both through toCitySlug is what makes that comparison hold;
   * the plain lower-case compare it replaced silently dropped the two
   * municipalities whose key and slug differ.
   */
  it("makes request municipality and provider city comparable", () => {
    const divergent = GLATKO_CITIES.filter((c) => c.key !== c.slug).map((c) => c.key);
    expect(divergent).toEqual(["hercegNovi", "bijeloPolje"]);

    for (const c of GLATKO_CITIES) {
      expect(toCitySlug(c.key), `dispatch ${c.key}`).toBe(toCitySlug(c.slug));
    }
    // The pairing the old compare got wrong.
    expect("hercegNovi".toLowerCase()).not.toBe("herceg-novi");
    expect(toCitySlug("hercegNovi")).toBe(toCitySlug("herceg-novi"));
  });
});
